...
2. Failure at a round time (30s/60s) behind nginx ⇒ read the LIVE config, NOT the repo's: `ssh shaman_web "sudo nginx -T" | grep -n proxy_read_timeout`. Prod loads `/etc/nginx/sites-enabled/shamanktmweb.conf` — a hand-edited regular file (not a symlink) that differs from `sites-available/` AND from `deploy/prod/nginx.conf`; no merge or deploy updates it.
