...
- Stock is never written through MCP. Balances change only via confirmed sales, order fulfilment, transfers, or an admin physical-count reconciliation in /sysuser/stock, so every movement keeps staff attribution. A mistake is corrected by a new reversing row, never an edit.
